...
Matrix multiplyMatrix(Matrix m1, Matrix m2){
    if(m1.colEff != m2.rowEff)return;
    Matrix m;
    createMatrix(m1.rowEff, m2.colEff, &m);
    for(int i = 0; i<m.rowEff;i++){
        for(int j = 0; j<m.colEff;j++){
            m.mem[i][j] = 0;
            for(int k = 0; k<m2.rowEff;k++){
...
boolean isSparse(Matrix m){
    ...
    float banyakBukanNol = (float)count/countElmt(m);
    return (banyakBukanNol<0.05);
...
float determinant(Matrix m){
    ...
    if(m.rowEff == 2)return (m.mem[0][0] * m.mem[1][1]) - (m.mem[0][1] * m.mem[1][0]);
    float det = 0;
    for(int i = 0; i < m.rowEff; i++){
        Matrix subMatrix;
        ...
        for(int j = 1; j < m.rowEff; j++){
            for(int k = 0; k < m.colEff; k++){
                if(k < i){
                    subMatrix.mem[j-1][k] = m.mem[j][k];
                }else if(k > i){
                    subMatrix.mem[j-1][k-1] = m.mem[j][k];
                }
            }
        }
        det += (i % 2 == 0 ? 1 : -1) * m.mem[0][i] * determinant(subMatrix);
    }
    return det;
}
...
Matrix transpose(Matrix m){
    Matrix mOut;
    createMatrix(m.colEff, m.rowEff, &mOut);
    for (int i = 0; i < m.rowEff; i++){
        for (int j = 0; j < m.colEff; j++){
            mOut.mem[j][i] = m.mem[i][j];
        }
    }
    return mOut;
