...
void ReadMatrix(Matrix *M){
    ...
    {
        scanf("%d %d",&row,&col);
        if ((row<=MAX_ROWS &&row>=1)&&(col<=MAX_COLS&&col>=1)){
...
/*      Meminta input ukuran matrix (rows, cols) hingga valid
        Valid: 0 < rows < MAX_ROW S and 0 < cols < MAX_COLS */
...
bool IsDiagonallyDominant(Matrix M){
    ...
    for (int i = 0; i < M.rows; i++) {
        ...
        for (int j = 0; j < M.cols; j++) {
            if (i != j) {
                sum += (float)fabs(M.data[i][j]);
            }
        }
        if ((float)fabs(M.data[i][i]) <= sum) {
            return false;
        }
    }
    return true;
