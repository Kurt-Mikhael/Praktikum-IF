 ...
 void AddPolynomials(Polynomial *p1, Polynomial *p2, Polynomial *result){
   ...
   for (int i = 0; i <= maxDeg; i++) {
       result->coeff[i] = GetCoefficient(p1, i) + GetCoefficient(p2, i);
   }
 ...
 void PrintPolynomial(Polynomial *p){
   ...
   for (int i = p->degree; i >= 0; i--) {
      ...
      if(i > 1){
         if(abs_coeff == 1) printf("x^%d", i);
         else printf("%dx^%d", abs_coeff, i);
      }
      else if(i==1){
         if(abs_coeff == 1) printf("x");
         else printf("%dx", abs_coeff);
      }
      else if(i == 0){
         printf("%d", abs_coeff);
      }
